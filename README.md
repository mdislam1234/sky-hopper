...
Vercel CLI 59.25.0 is installed. Deployment is waiting for the user's `vercel login`: the CLI reports logged out. The planned method is to deploy only the compiled static `build/web` directory with no remote Flutter installation or unnecessary route rewrites, following current Vercel static-deployment guidance. No Vercel project/deployment, production OAuth changes, live deployment tests, split release APKs or final submission ZIP have been completed yet. Resume by verifying Vercel authentication and existing projects, then deploy the already-built output.
